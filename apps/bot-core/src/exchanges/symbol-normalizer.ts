...
export interface NormalizedPair {
  ...
  exchangeSymbols: Map<string, string>; // exchangeId -> native symbol
}

export class SymbolNormalizer {
  private normalizedPairs = new Map<string, NormalizedPair>();
  
  // Common asset mappings across exchanges
  private assetMappings = new Map<string, string>([
  ...
  private quoteAssetPriority = new Map<string, number>([
    ['USD', 10],
    ['USDT', 9],
    ['USDC', 8],
    ['EUR', 7],
  ...
  getAvailableSymbols(minExchanges: number = 2): string[] {
  ...
  private extractQuoteAsset(normalizedSymbol: string): string {
  ...
  getStats(): {
    ...
    const exchangeCoverage = new Map<string, number>();
